...
namespace PepijnWillekens.EasyWallColliderUnity {
    ...
    public class EasyWallCollider : MonoBehaviour {
        ...
        public void MakeAllEdgeColliders(List<Vector2> corners) {
            for (int i = 0; i < corners.Count + loopInt(); i++) {
                MakeEdgeCollider(string.Format("{0} - {1}", i + 1, ((i + 1) % corners.Count) + 1), corners[i],
                    corners[(i + 1) % corners.Count], radius, heigth, extraWidth);
            }
        }

        public void MakeAllCornerColliders(List<Vector2> corners) {
            for (int i = 0; i < corners.Count; i++) {
                MakeCornerCollider(string.Format("{0}", i + 1), corners[i], radius, heigth);
            }
        }

        private void MakeEdgeCollider(string name, Vector2 from, Vector2 to, float radius, float heigth,
                                      float extraWidth) {
            ...
            go.transform.localPosition =
                To3D((from + to) / 2 + Perp(from, to, true) * extraWidth / 2).ChangeY(heigth / 2);
            go.transform.forward = transform.TransformDirection(To3D((to - from).normalized));
            go.transform.localScale = new Vector3(radius * 2 + extraWidth, heigth, Vector2.Distance(from, to));
        }

        private void MakeCornerCollider(string name, Vector2 current, float radius, float heigth) {
            var go = new GameObject("CornerCollider " + name);
            go.layer = gameObject.layer;
            go.isStatic = gameObject.isStatic;
            go.transform.SetParent(ColliderContainer);
            var collider = go.AddComponent<CapsuleCollider>();
            go.transform.localPosition = To3D(current).ChangeY(heigth / 2);
            ...
            if (makeRenderers) {
                ...
                r.transform.localPosition = To3D(current).ChangeY(heigth / 2);
        ...
        private void OnDrawGizmos() {
            if (!onlyWhenSelected) DrawGizmos();
        }
        ...
        void DrawBar(Vector3 pos) {
            DrawLine(pos.ChangeY(0), pos.ChangeY(heigth));
        }
